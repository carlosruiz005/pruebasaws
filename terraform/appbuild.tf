...
resource "null_resource" "pruebas_springboot" {
  provisioner "local-exec" {
    command = <<EOT

	cd ../pruebas/ && mvn clean package -Dmaven.test.skip=true
    	docker build -t pruebas . 
    	docker tag pruebas ${data.aws_ecr_repository.image_repo.repository_url}
...
depends_on = [aws_ecr_repository.pruebas]
}
